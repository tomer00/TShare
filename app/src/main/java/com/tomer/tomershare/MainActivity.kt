...
import com.tomer.tomershare.databinding.DeleteDaiBinding
...
class MainActivity : AppCompatActivity(), AdaptApp.AppClickLis, AdaptGal.GalClickLis, AdaptFiles.FIleClickLis {
    ...
    private val adapGal by lazy { AdaptGal(this, repo.getAllImusVideos()) }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        b.apply {
            ...
            btRec.setOnClickListener {
                it.haptic()
                val i = Intent(this@MainActivity, TransNew::class.java)
                i.putExtra("rec", "rec")
                startActivity(i)
            ...
            btSend.setOnClickListener {
                if (Utils.files.isNotEmpty()) {
                    startActivity(Intent(this@MainActivity, TransNew::class.java))
                    overridePendingTransition(0, R.anim.exit_d)
                    if (isGal) {
                        galPos.forEach {
    ...
    override fun onResume() {
        super.onResume()
        b.tvItems.text = Utils.files.size.toString()
    }
    ...
    override fun onGalClick(position: Int, img: ImageView) {
        ...
        if (!f.visi) {
            ...
            Utils.files.add(AppModal(f.file.name, "0", f.file, drDef!!))
    ...
    override fun onFileClick(position: Int, indic: View, thumb: ImageView) {
        ...
        if (f.isDirectory) {
            ...
            if (indic.visibility == View.GONE) {
                Utils.files.add(AppModal(f.name, "0", f, drDef!!))
...
//region HELPER FUNCTIONS
    ...
    private fun getDelDia(): AlertDialog {
        val fb: DeleteDaiBinding = DeleteDaiBinding.inflate(LayoutInflater.from(this))
...
//region HASHMAP FOR DRAWABLE FOR DIFFERENT TYPES
    ...
    private fun getDrawables(): List<Drawable> {
        ...
        ContextCompat.getDrawable(this, R.drawable.ic_vid)?.let { dr.add(it) }
